	...
	$.getJSON('http://<?php echo $_SERVER['HTTP_HOST']; ?>/json/searchusers.php?key=' + encodeURIComponent(updateKey) + "&query=" + encodeURIComponent( $('#userSearchInput').val() ), function(data){
		...
		if(data.result == true && data.key == updateKey)
		{
			...
			else
			{
				...
				{
					$('#userSearchContent').append("<b>" + data.users[i].firstname + ' "' + data.users[i].nick + '" ' + data.users[i].lastname + '</b> <input type="button" value="Velg" onclick="setSelectedUser(\'' + data.users[i].userId + '\', \'' + data.users[i].firstname + ' &quot;' + data.users[i].nickname + '&quot; ' + data.users[i].lastname + '\')" /><br />');
				}
			}
		}
  	});
