				...
				if (Session::isAuthenticated()) {
					...
					echo '<div id="container">';
						...
						echo '<div class="banner" id="nav">';
							...
							//Show a small bubble if the user has an incoming friend request
							$friendList = $user->getPendingFriendsTo();
							echo '<a href="index.php?page=friends"><h1' . ($pageString == 'friends' ? $underlined : '') . '>Venner' . (count($friendList)==0?'':'<span class="friendBubble">' . count($friendList) . '</span>') . '</h1></a>';
									...
									echo '<ul id="ul1">';
										...
										echo '<div id="registerFrame" style="display:none;">';
											...
											echo '<form class="register" method="post">';
												...
												echo '</li>';
												echo '<li>';
													echo '<input type="email" name="confirmemail" placeholder="Gjenta e-post" required>';
												echo '</li>';
												echo '<li>';
